    ...
    if timeup then
        if self.target.func ~= nil then
            ...
            if self.obj_not_nil then
                status, err = pcall(self.target.func, self.target.obj, table.unpack(self.param))
            else
                status, err = pcall(self.target.func, table.unpack(self.param))
            end
            if not status then
                self.over = true
                Log.Error(err)
