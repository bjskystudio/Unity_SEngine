...
local TypeEnum = lazyRequire("TypeEnum")
local Log = lazyRequire("Log")
...
local function AddListener(self, eventID, handler, obj, sendKey)
    if not eventID or (type(eventID) ~= TypeEnum.Num and type(eventID) ~= TypeEnum.Str) then
        error("event has to be string, " .. type(eventID) .. " not right.")
    end

    if not handler or type(handler) ~= TypeEnum.Func then
