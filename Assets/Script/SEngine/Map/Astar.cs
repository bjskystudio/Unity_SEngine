...
namespace SEngine.Map
{
    ...
    public class Astar
    {
        ...
        public Astar(int[] maze, BoundsInt bounds)
        {
            if (maze.Length == 0
                || bounds == null
                || bounds.size.x <= 0
        ...
        public bool GetPathByCell(PointList pathPoints, Vector3Int startPoint, Vector3Int endPoint, bool isIgnoreCorner = false)
        {
            ...
            {
                //Debug.Log(pathPoints[i].Info[0]);
                //Debug.Log(pathPoints[i].Info[1]);
            }
        ...
        bool isCanreach(AStarPoint point, AStarPoint target, bool isIgnoreCorner) //判断某点是否可以用于下一步判断
        {
            ...
            var c = vMaze[nTargetIndex];
            var d = (int) Enum.EnPhyFlag.CANT_MOVE;
            var a = (vMaze[nTargetIndex] & (int) Enum.EnPhyFlag.CANT_MOVE);
            var b = (target.Info[0] == point.Info[0] && target.Info[1] == point.Info[1]);
            
            if (nTargetIndex < 0
                || nTargetIndex >= nSize
                || (vMaze[nTargetIndex] & (int)Enum.EnPhyFlag.CANT_MOVE) != 0
            ...
            else
            {
                ...
                else
                {
                    ...
                    //斜对角要判断是否绊住
                    if ((vMaze[nPxTyIndex] & (int)Enum.EnPhyFlag.CANT_MOVE) == 0
        ...
        public List<Vector3Int> GetPath(Enum.EnTileMapType tilemap , Vector3Int startPos, Vector3Int endPos, bool isIgnoreCorner = false)
        {
            ...
            if (mapService.CellPosCanMove(startPos,tilemap) == false || mapService.CellPosCanMove(endPos,tilemap) == false)
            {
            ...
            if (mapService.CellPosIsInPutArea(startPos,tilemap) && mapService.CellPosIsInPutArea(endPos,tilemap) == false)
            {
                /*
