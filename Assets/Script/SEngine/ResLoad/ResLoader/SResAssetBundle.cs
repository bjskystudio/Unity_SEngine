...
{
    [Serializable]
    public class SResAssetBundle : SRes
    {
        ...
        protected override IEnumerator CoLoadByAB(bool isSync, bool isAll, bool isPreLoad, Action<System.Object, SResRef> callback)
        {
        ...
        public void AddDepRef()
        {
            for (int i = 0; i < DepList.Count; i++)
            {      
                if (ResLoadManager.Instance.mResMap.ContainsKey(DepList[i]))
                {
                    SResAssetBundle res = ResLoadManager.Instance.mResMap[DepList[i]] as SResAssetBundle;
                    res.OnAddRef();
                }
            }
        }
        ...
        public static void AddWhoRefMe(string me, string refMe)
        {
            ...
            if(ResLoadManager.Instance.mResMap.ContainsKey(me))
            {
                SResAssetBundle hAB = ResLoadManager.Instance.mResMap[me] as SResAssetBundle;
