...
public sealed class UIGoTable : MonoBehaviour
{
    ...
    [CSharpCallLua]
    public delegate void LuaOnClickTmpAction(LuaTable t, STMP g, string linkId);
        ...
        for (int i = 0; i < uiNodeArray.Length; i++)
        {
            ...
            if (nodeinfo.Obj != null)
            {
                if (nodeinfo.gameObject)
                {
                    ...
                    var tmp = nodeinfo.gameObject.GetComponent<STMP>();
                    if (tmp != null)
                    {
                        uiTmpList.Add(tmp);
                    }
    ...
    private void OnTmpClick(STMP go, PointerEventData eventData, string linkId)
    {
