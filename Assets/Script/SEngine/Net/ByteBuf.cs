...
namespace SEngine.Net
{
	public struct ByteBuf
	{
		...
		{
			data = TSArrayPool<byte>.Get(capacity, true);
			realLength = capacity;
			readerIndex = 0;
			writerIndex = 0;
			markReader = 0;
		...
		public ByteBuf Capacity (int nc)
		{
			if (nc > realLength)
			{
				...
				{
					byte[] old = data;
					var oldRealLength = realLength;
					data = TSArrayPool<byte>.Get(nc, true);
					realLength = nc;
					Array.Copy (old, data, oldRealLength);
					TSArrayPool<byte>.Release(old);
				}
			}
	    ...
	    **/
		...
		public ByteBuf Copy()
		{
			ByteBuf item = new ByteBuf(realLength);
			Array.Copy (this.data, item.data, this.realLength);
		...
		{
			if (readerIndex >= 0 && readerIndex <= writerIndex && writerIndex <= realLength)
			{
				this.readerIndex = readerIndex;
        ...
        public ByteBuf WriteUShortLE(ushort value)
        {
            Capacity(writerIndex + 2);
            data[writerIndex++] = (byte)(value & 0xff);
            data[writerIndex++] = (byte)((value >> 8) & 0xff);
		 ...
		 * 写入一部分字节
		 **/
        public ByteBuf WriteBytes(ref ByteBuf src)
		{
		 ...
		 * 写入一部分字节
		 **/ 
		public ByteBuf WriteBytes(ref ByteBuf src ,int len)
		{
			if (len > 0)
			{
		...
		{
			if (writerIndex >= readerIndex && writerIndex <= realLength)
			{
				this.writerIndex = writerIndex;
			}
			return this;
